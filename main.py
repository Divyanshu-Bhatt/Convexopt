...
def main(args):
    ...
    val_loader = torch.load(os.path.join(args.data_dir, "valloader.pth"))
    test_loader = torch.load(os.path.join(args.data_dir, "testloader.pth"))

    network_credentials = model_initialisation(args)
    model_train(network_credentials, train_loader, val_loader, test_loader,
                args)

    with open(os.path.join(args.model_save_dir, "config.json"), "w") as file:
        json.dump(vars(args), file)

    with open(os.path.join(args.common_loc, "model_config.json"), "w") as file:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--hidden_dims', nargs='+', required=True)
    ...
    args.in_dim = data_arguments["in_dim"]
